...
public class DataSourceView extends HBox {
	...
	Label labelY;
	Button buttonMvo;
	Button buttonVerwijderen;

	public DataSourceView(MvoGoalController mvoGoalController, MvoGoalChild mvoGoal, HoofdPaneel hoofdPaneel,
			DatasourceController datasourceController, Datasource datasource, int mvoId) {
	...
	public void maakTableView(Datasource datasource) {
		...
		buttonVerwijderen = new Button("Datasource verwijderen");
		buttonVerwijderen.setId("terugmvobtn_id");
		buttonVerwijderen.setOnAction(e -> {

			Alert alert = new Alert(AlertType.CONFIRMATION);
			alert.setTitle("Confirmeer verwijdering");
			alert.setHeaderText("Bent u zeker dat u deze datasource wilt verwijderen?");
			alert.setGraphic(null);

			Optional<ButtonType> result = alert.showAndWait();
			if (result.isPresent() && result.get() == ButtonType.OK) {
				mvoGoal.removeDatasource();
				mvoGoalController.updateMvoGoal( mvoGoal);
				
				datasourceController.deleteDatasource(datasource);
		...
		vbox.getChildren().add(buttonVerwijderen);
		...
		listViewX.setBorder(new Border(
				new BorderStroke(Color.BLACK, BorderStrokeStyle.SOLID, CornerRadii.EMPTY, BorderWidths.DEFAULT)));
		listViewY.setBorder(new Border(
				new BorderStroke(Color.BLACK, BorderStrokeStyle.SOLID, CornerRadii.EMPTY, BorderWidths.DEFAULT)));
		...
		listViewX.setOnEditCommit(new EventHandler<ListView.EditEvent<String>>() {
			...
			public void handle(EditEvent<String> event) {
				...
				datasource.getData().getDataSet().put(listViewY.getSelectionModel().getSelectedItem(),
		...
		listViewY.setOnEditCommit(new EventHandler<ListView.EditEvent<String>>() {
			...
			public void handle(EditEvent<String> event) {
				...
				datasource.getData().getDataSet().put(event.getNewValue(),
						Integer.parseInt(listViewX.getSelectionModel().getSelectedItem()));
		...
		expectedGoalColumn
				.setCellFactory(TextFieldTableCell.<Datasource, Integer>forTableColumn(new IntegerStringConverter()));
